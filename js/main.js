jQuery(document).ready(function($){
	...
	$('.floating-img').find('div').eq(0).load(function() {
	...
	//on resize - adjust .container-fluid and .floating-img dimentions and position
	$(window).on('resize', function(){
		if( $('html').hasClass('preserve-3d') ) {
			...
			$('.floating-img').attr('style', '').removeClass('is-absolute');
		}
	});
	...
	function initBackground() {
		...
		//set an height for the .container-fluid
		$('.container-fluid').css({
			'height' : wrapperHeight,
		});
		//set dimentions and position of the .container-fluid
		$('.floating-img').addClass('is-absolute').css({
			'left' : newLeft,
			'top' : newTop,
			'width' : newImageWidth,
		});
